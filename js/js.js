...
// TIMER ANIMATE
function timerimg() {
    var img = 90, imgMax = 149, imgMin = 90;
    var npr = 0;
...
document.querySelectorAll('.burger').forEach(el => el.addEventListener('click', event => {
...
$(window).on('scroll', function () {
    ...
    if ($('#s-3-1').length) {
        if (stop < $('#s-3-1').offset().top + $('#s-3-1').height() + $('#s-3-2').height() + 40 - vihe) {
            $('#s-3-2').addClass('pfb');
        } else {
    ...
    if ($('.section-2').length) {
        if (stop < $('.section-2').offset().top) {
            ...
            if ($(".b-2").length) {
                ...
                var otop = 50;
                if (stop > $(".b-2").offset().top + otop && stop < ($(".b-2").offset().top + $(".b-2").height() - vihe)) {
                    $(".screen").removeClass('active');
                    $(".b-2").addClass('active');
                    let pos = ((($(".b-2 .content").offset().top - otop) * screent1) / ($(".b-2").height() - vihe - otop)).toFixed(0);
                    var context = document.querySelector(".b-2 .content canvas").getContext('2d');
                    if (screen1[pos]) {
                        context.reset();
            ...
            if ($(".b-3").length) {
                ...
                if (stop > $(".b-3").offset().top + otop && stop < smax) {
                    $(".screen").removeClass('active');
                    $(".b-3").addClass('active');
                    pos = ((($(".b-3 .content").position().top - otop) * screent2) / ($(".b-3").height() - vihe - otop)).toFixed(0);
                }
                if (stop == smax) pos = screent2;
                var context = document.querySelector(".b-3 .content canvas").getContext('2d');
                if (screen2[pos]) {
                    context.reset();
            ...
            if ($(".b-4").length) {
                ...
                if (stop > $(".b-4").offset().top + otop && stop < smax) {
                    $(".screen").removeClass('active');
                    $(".b-4").addClass('active');
                    pos = ((($(".b-4 .content").position().top - otop) * screent3) / ($(".b-4").height() - vihe - otop)).toFixed(0);
                }
                if (stop == smax) pos = screent3;
                var context = document.querySelector(".b-4 .content canvas").getContext('2d');
                if (screen3[pos]) {
                    context.reset();
            ...
            if ($(".b-5").length) {
                ...
                if (stop > $(".b-5").offset().top + otop && stop < smax) {
                    $(".screen").removeClass('active');
                    $(".b-5").addClass('active');
                    pos = ((($(".b-5 .content").position().top - otop) * screent4) / ($(".b-5").height() - vihe - otop)).toFixed(0);
                ...
                var context = document.querySelector(".b-5 .content canvas").getContext('2d');
                if (screen4[pos]) {
                    context.reset();
            ...
            if (document.querySelector(".section-1 .bg canvas")) document.querySelector(".section-1 .bg canvas").getContext('2d').reset();
            if (document.querySelector(".b-2 .bg canvas")) document.querySelector(".b-2 .bg canvas").getContext('2d').reset();
            if (document.querySelector(".b-2 .content canvas")) document.querySelector(".b-2 .content canvas").getContext('2d').reset();
            if (document.querySelector(".b-3 .content canvas")) document.querySelector(".b-3 .content canvas").getContext('2d').reset();
            if (document.querySelector(".b-4 .content canvas")) document.querySelector(".b-4 .content canvas").getContext('2d').reset();
            if (document.querySelector(".b-5 .content canvas")) document.querySelector(".b-5 .content canvas").getContext('2d').reset();
        }
    }
    // if ($(".section-5").length) {
    ...
    if ($(".section-5").length) {
        ...
        if (stop > (secTop - vihe) && stop < (secTop + secH)) {
            var xx = stop - (secTop - vihe);
            xx = (xx / 4).toFixed(2);

            $('.section-5 .container-box .imgs > div[class^="img-"]').each(function () {
                const $el = $(this);
                ...
                if (tf && tf !== 'none') {
                    ...
                    if (m) {
                        ...
                        if (!isNaN(a) && !isNaN(d)) {
                            scale = (Math.abs(a) + Math.abs(d)) / 2 || 1;
                        }
                    } else {
                        const s = tf.match(/scale\(([\d.]+)\)/);
                        if (s) scale = parseFloat(s[1]) || 1;
                    }
                }

                const speed = scale < 1 ? 0.5 : 1;

                const tx = - (parseFloat(xx) * speed).toFixed(2);

                if (scale !== 1) {
                    $el.css('transform', `translateX(${tx}px) scale(${scale})`);
                } else {
    ...
    if ($(".footer-box .bgs .before").length) {
        ...
        $(".footer-box .bgs .before").css('transform', 'translate(-' + trt + 'px, ' + trt + 'px)');
        $(".footer-box .bgs .after").css('transform', 'translate(' + trt + 'px, -' + trt + 'px)');
    }
...
function setupMarquee(marqueeContent) {
    ...
    marqueeContent.style.setProperty("animation", `scrolling-${direction === true ? 'right' : 'left'} var(--marquee-animation-duration) linear infinite`);
}
...
function updateElements(attributes) {
    ...
    if (textsCards.value) {
        ...
        if (cardsContainer) {
            ...
            function equalizeTextLines({ maxLinesCap = 6 } = {}) {
                const texts = Array.from(document.querySelectorAll('.b-5 .mission .item .text'));
            ...
            if (document.fonts?.ready) document.fonts.ready.then(() => equalizeTextLines());
            window.addEventListener('resize', () => equalizeTextLines());
        }
    }

    const testimonialCards = attributes["testimonials"].find(({ key }) => key === "cards");
    if (testimonialCards.value) {
        ...
        const cardsContainer = document.querySelector(`[data-key="testimonials:cards"]`);
        if (cardsContainer) {
            cardsContainer.innerHTML = "";
    ...
    const navigationLinks = attributes["navigation"].find(({ key }) => key === "website_links");
 ...
 if (linksContainers) {
    ...
    if (socialLinks.value) {
        // With sublinks
        // <div class="socio-box-item dropdown-menu up">
        //     <a class="socio-link dropdown-menu-switch">
        //         <img src="img/socio-1.svg" alt="telegram">
        //     </a>
        //
        //     <ul>
        //         <li><a target="_blank" href="https://t.me/+-8FiECKBiBo4MTk1">Global</a></li>
        //         <li><a target="_blank" href="https://t.me/bluwhalecis">CIS</a></li>
        //         <li><a target="_blank" href="https://t.me/bluwhaletr">Turkey</a></li>
        //     </ul>
        // </div>
        // Without sublinks
        // <div class="socio-box-item">
        //     <a class="socio-link" target="_blank" href="https://www.linkedin.com/company/bluwhaleai"><img
        //         src="img/socio-2.svg"
        //         alt="linkedin"></a>
        // </div>
        const socialContainers = document.querySelectorAll(`[data-key="navigation:social_links"]`);
        if (socialContainers) {
            socialContainers.forEach((container) => {
                ...
                socialLinks.value.forEach(({text, link, icon, sublinks}) => {
                    ...
                    if (sublinks && sublinks.length > 0) {
                        ...
                        const links = sublinks.map(x => `<li><a target="${x.external ? "_blank" : ""}" href="${x.link}">${x.text}</a></li>`)
        ...
        // <div class="socio-box-item dropdown-menu up">
        //     <a class="socio-link dropdown-menu-switch">
        //         <div class="img"><img src="img/socio-1.svg" alt="telegram"></div>
        //         <span>TElEGraM</span>
        //     </a>
        //
        //     <ul>
        ...
        // Wihtout sublinks
        // <div class="socio-box-item">
        //     <a target="_blank" class="socio-link" href="https://www.linkedin.com/company/bluwhaleai">
        //         <div class="img"><img src="img/socio-2.svg" alt="linkedin"></div>
        //         <span>LinkedIn</span></a>
        // </div>
        const socialWithTextContainers = document.querySelectorAll(`[data-key="navigation:social_links-with-text"]`);
            ...
            socialWithTextContainers.forEach((container) => {
                ...
                socialLinks.value.forEach(({text, link, icon, sublinks}) => {
                    ...
                    if (sublinks && sublinks.length > 0) {
                        ...
                        const links = sublinks.map(x => `<li><a target="${x.external ? "_blank" : ""}" href="${x.link}">${x.text}</a></li>`)
                        const linksStr = links.join("")
                        divElement.innerHTML = `<a class="socio-link dropdown-menu-switch"><div class="img"><img src="${icon}" alt="${text}"></div><span>${text}</span></a><ul>${linksStr}</ul>`
                    } else {
                        divElement.className = "socio-box-item";
                        divElement.innerHTML = `<a target="_blank" class="socio-link" href="${link}"><div class="img"><img src="${icon}" alt="${text}"></div><span>${text}</span></a>`
                    }
                    container.appendChild(divElement);
                });
            });
    ...
    const investor_logos_line_1 = attributes["logos"].find(({ key }) => key === "investor_logos_line_1");
    const investor_logos_line_1_height = attributes["logos"].find(({ key }) => key === "investor_logos_line_1_height");
    const investor_logos_line_1_animation_duration = attributes["logos"].find(({ key }) => key === "investor_logos_line_1_animation_duration");

    const investor_logos_line_1_copies = attributes["logos"].find(({ key }) => key === "investor_logos_line_1_copies");
    const investor_logos_line_1_direction_right = attributes["logos"].find(({ key }) => key === "investor_logos_line_1_direction_right");
    marqueeCopies["logos:investor_logos_line_1"] = investor_logos_line_1_copies.value
    marqueeDirections["logos:investor_logos_line_1"] = investor_logos_line_1_direction_right.value

    if (investor_logos_line_1.value && investor_logos_line_1.value.length > 0) {
        // <li><img src="img/logos/line-1/L1-1.svg" alt=""/></li>
        const marqueeContentL1 = document.querySelector(`[data-key="logos:investor_logos_line_1"]`);
        if (marqueeContentL1) {
            ...
            if (investor_logos_line_1_height.value) {
                marqueeContentL1.parentElement.style.setProperty("--marquee-height", investor_logos_line_1_height.value + 'px');
            }

            if (investor_logos_line_1_animation_duration.value) {
                marqueeContentL1.parentElement.style.setProperty("--marquee-animation-duration", investor_logos_line_1_animation_duration.value + 's');
            }

            if (investor_logos_line_1_copies.value) {
                marqueeContentL1.parentElement.style.setProperty("--marquee-copies", investor_logos_line_1_copies.value);
            }
        }
    }

    const investor_logos_line_2 = attributes["logos"].find(({ key }) => key === "investor_logos_line_2");
    const investor_logos_line_2_height = attributes["logos"].find(({ key }) => key === "investor_logos_line_2_height");
    const investor_logos_line_2_animation_duration = attributes["logos"].find(({ key }) => key === "investor_logos_line_2_animation_duration");

    const investor_logos_line_2_copies = attributes["logos"].find(({ key }) => key === "investor_logos_line_2_copies");
    const investor_logos_line_2_direction_right = attributes["logos"].find(({ key }) => key === "investor_logos_line_2_direction_right");
    marqueeCopies["logos:investor_logos_line_2"] = investor_logos_line_2_copies.value
    marqueeDirections["logos:investor_logos_line_2"] = investor_logos_line_2_direction_right.value
    if (investor_logos_line_2.value && investor_logos_line_2.value.length > 0) {
        // <li><img src="img/logos/line-1/L1-1.svg" alt=""/></li>
        const marqueeContentL2 = document.querySelector(`[data-key="logos:investor_logos_line_2"]`);
        if (marqueeContentL2) {
            ...
            if (investor_logos_line_2_height.value) {
                marqueeContentL2.parentElement.style.setProperty("--marquee-height", investor_logos_line_2_height.value + 'px');
            }

            if (investor_logos_line_2_animation_duration.value) {
                marqueeContentL2.parentElement.style.setProperty("--marquee-animation-duration", investor_logos_line_2_animation_duration.value + 's');
            }

            if (investor_logos_line_2_copies.value) {
                marqueeContentL2.parentElement.style.setProperty("--marquee-copies", investor_logos_line_2_copies.value);
            }
        }
    }

    const partner_logos = attributes["logos"].find(({ key }) => key === "partner_logos");
    const partner_logos_height = attributes["logos"].find(({ key }) => key === "partner_logos_height");
    const partner_logos_animation_duration = attributes["logos"].find(({ key }) => key === "partner_logos_animation_duration");

    const partner_logos_copies = attributes["logos"].find(({ key }) => key === "partner_logos_copies");
    const partner_logos_direction_right = attributes["logos"].find(({ key }) => key === "partner_logos_direction_right");
    marqueeCopies["logos:partner_logos"] = partner_logos_copies.value
    marqueeDirections["logos:partner_logos"] = partner_logos_direction_right.value
    if (partner_logos.value && partner_logos.value.length > 0) {
        // <li><img src="img/logos/line-1/L1-1.svg" alt=""/></li>
        const marqueeContentP = document.querySelector(`[data-key="logos:partner_logos"]`);
        if (marqueeContentP) {
            ...
            if (partner_logos_height.value) {
                marqueeContentP.parentElement.style.setProperty("--marquee-height", partner_logos_height.value + 'px');
            }

            if (partner_logos_animation_duration.value) {
                marqueeContentP.parentElement.style.setProperty("--marquee-animation-duration", partner_logos_animation_duration.value + 's');
            }

            if (partner_logos_copies.value) {
                marqueeContentP.parentElement.style.setProperty("--marquee-copies", partner_logos_copies.value);
            }
        }
    }
...
if (media_logos_link_text_gradient.value) {
    ...
    if (media_logos.value) {
        ...
        if (imgsElement) {
            imgsElement.innerHTML = media_logos.value.map((img, index) => `
                <div class="img-${index + 1}"><img src="${img.image}" alt=""></div>
            `).join('');
            ...
            media_logos.value.forEach((img, index) => {
                ...
                if (imgWrapper) {
                    ...
                    const top = isSmallScreen ? img?.override_position_top_small : img?.override_position_top;
                    const left = isSmallScreen ? img?.override_position_left_small : img?.override_position_left;
